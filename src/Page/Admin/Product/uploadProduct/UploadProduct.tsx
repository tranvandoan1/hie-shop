...
const UploadProduct: React.FC = ({ callBack, state, data }: Props) => {
  ...
  const newClassifies = classifies?.value?.filter(
    (item: any) => item.linked == productsValue?.linked
  );
  // lấy tên phân loại
  const classifyValue1: any = [];
  newClassifies?.map((item: any) => classifyValue1.push(item.name));
  ...
  useEffect(() => {
    ...
    const newClassifies2: any = [];
    newClassifies?.map((item: any) => {
    ...
    const uniqueNewClassifies2 = newClassifies2?.filter((value: any, index: any) => {
      return newClassifies2?.indexOf(value) === index;
    });

    // loại bỏ tên trùng nhau
    const uniqueClassifies2Value: any = [];
    newClassifies2?.filter((item: any, index: any) => {
    ...
    const duplicateMame = (duplicateMameArr: any = []) => {
      const newData: any = [];
      while (duplicateMameArr.length > 0) {
        newData.push(duplicateMameArr[0]);
        duplicateMameArr = duplicateMameArr?.filter(
          (item: any) => item.name !== duplicateMameArr[0].name
        );
      }
      return newData;
    };

    const newClassifiesValue: any = [];
    newClassifies?.map((item: any) => {
                ...
                <Select
                  ...
                  defaultValue={
                    categories?.data?.find(
                      (item: any) => item._id == dataValue?.cate_id
                    )?._id
                  }
                >
