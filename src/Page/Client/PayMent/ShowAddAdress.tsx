...
const ShowAddAddress = ({ isModalOpen, setIsModalOpen, infoUsers }: Props) => {
  ...
  const [uploadAdress, setUploadAdres] = useState<any>({
    status: false,
    data: undefined,
    close: true,
  }); //upload địa chỉ
  const [dataUrlCity, setDataUrlCity] = useState();
  const [city, setCity] = useState<any>(); //thành phố
  const [district, setDistrict] = useState<any>(); //huyện
  const [ward, setWard] = useState<any>(); //xá
  const [showTags, setShowTags] = useState<number>(1); //hiện thành phố huyện xã từ api
  const [specificAddress, setSpecificAddress] = useState<any>(); //địa chỉ cụ thể
  ...
  const onChange = async (e: any) => {
    setLoading(true);
    const newData = infoUsers?.find((item: any) => item.status == true);
    await dispatch(
