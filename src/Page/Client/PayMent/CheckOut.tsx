...
const CheckOut = () => {
    ...
    const infoUsers = useSelector((data: any) => data.infoUsers.value);
    const users = useSelector((data: any) => data.users.value)
    const user = users?.data?.find((item: any) => item._id == getDataUserLoca()?._id);
    const userShop=users?.data?.find((item:any)=>item.code==getDataUserLoca()?.code)
    
    const dataAdressUser = infoUsers?.find((item: any) => item.status == true);
