...
// @ts-ignore
import { uploadSaveOrder } from "../../../features/SaveOrderSlice";
// @ts-ignore
import { getProductAll } from "../../../features/Products";
import Footer from "../../../components/Footer";
import ShowAddAddress from "../PayMent/ShowAddAdress";
...
const Cart = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "name_pro",
      key: "name_pro",
      render: (name_pro) => <span>{name_pro}</span>,
    },
    {
  ...
  // thêm bớt số lượng
  const selectValuePro = async (item) => {
    ...
    if (item.value.amount > 1) {
      ...
      const newData = selectOrder?.filter((itemValue) => itemValue._id !== item.value._id)
      let sum = 0
